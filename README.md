...
* **Anchor Convergence Residual:** Calibrated via iterative optimization from an initial $40.93\text{ km}$ displacement down to a sub-meter anchor error ($0.5\text{ m}$)[cite: 2].
* **Metocean Format:** Structured NetCDF-4 binary grid storing multi-dimensional spatial arrays[cite: 2]:
  * Current velocity components ($u_{\text{current}}, v_{\text{current}}$)[cite: 2]
  * Atmospheric boundary wind vectors ($u_{10}, v_{10}$)[cite: 2]
  * Dimensional bounds: $49\text{ time intervals} \times 21\text{ latitudes} \times 21\text{ longitudes}$[cite: 2]

---

## 📊 Telemetry Verification & Vessel Attribution

Evaluated on historical AIS traffic fixes across an active commercial corridor in the Arabian Sea[cite: 1, 2]:

| MMSI | Vessel Name | Vessel Classification | Fix Count | Speed Profile (SOG) | Forensic Classification |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `352004477` | MV ARABIAN STAR | Bulk Carrier[cite: 1] | 145 / 145[cite: 1] | Steady $12.5\text{ kts}$[cite: 1] | 🟢 **Clear** (Transit lane) |
| `419000321` | FS SAGAR KIRTI | Fishing Vessel[cite: 1] | 145 / 145[cite: 1] | Steady $3.8\text{ kts}$[cite: 1] | 🟢 **Clear** (Local trawler) |
| `419001234` | **MT OCEAN TITAN** | **Crude Oil Tanker**[cite: 1] | **125 / 145**[cite: 1] | **Plunged $14.0 \rightarrow 3.0\text{ kts}$**[cite: 1] | 🔴 **PRIMARY SUSPECT (Attributed)** |
| `419555001` | MV CORAL VENTURE | General Cargo[cite: 1] | 145 / 145[cite: 1] | Steady $11.0\text{ kts}$[cite: 1] | 🟡 **Witness** (Near corridor, nominal SOG) |
| `419001999` | MV KONKAN TRADER | Container Ship[cite: 1] | 145 / 145[cite: 1] | Steady $15.0\text{ kts}$[cite: 1] | 🟢 **Clear** (Transit lane) |
| `538071234` | MT SULPHUR PRIDE | Oil/Chem Tanker[cite: 1] | 145 / 145[cite: 1] | Steady $13.0\text{ kts}$[cite: 1] | 🟢 **Clear** (Transit lane) |
| `636092188` | ST OCEANIC HARMONY | Crude Oil Tanker[cite: 1] | 145 / 145[cite: 1] | Steady $14.0\text{ kts}$[cite: 1] | 🟢 **Clear** (Transit lane) |


```

[!] FORENSIC ATTRIBUTION REPORT:
• Suspect MMSI: 419001234 (MT OCEAN TITAN)
• Attribution Score: 75% Positive Weighting
• Identified Anomalies:
├─ 20-Fix (3.5-Hour) Intentional Transponder Blackout Window
├─ 11.0-Knot Speed Drop (Decelerated to 3.0 kts during discharge window)
...
* **Ocean Physics Engine:** OpenDrift Framework, CMEMS Currents API, NetCDF-4 Multidimensional Storage[cite: 2]
* **Spatial Backend & Telemetry:** PostgreSQL + PostGIS, SQLite[cite: 1], FastAPI
...
git clone [https://github.com/aegisx-maritime/aegisx.git](https://github.com/aegisx-maritime/aegisx.git)
cd aegisx
